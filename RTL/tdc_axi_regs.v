...
//   ★ 2026-09-15 4단계 : 교정표 RAM 창 0x2000 + i*4 (RW). 상세는 CAL_SEL_BIT 주석.
//
//  [클럭이 두 개인 이유]
...
module tdc_axi_regs #(
    ...
    // ★ 2026-09-16 : 7 -> 8. Mode 0 확인용 히트 생성기 주기 레지스터(M0PER, 0x48)를 추가했다.
    //   (2026-09-15 : 6 -> 7 교정표 창 0x2000, 2026-09-05 : 5 -> 6 DANGER). PS 는 BUILD[7:0] 로 확인한다.
    parameter [7:0]   REGMAP_VER         = 8'd8
)(
    ...
    // ★ 2026-09-15 추가 : 교정표 RAM 창 (전부 s_axi_aclk 도메인)
    //   tdc_calib_ram 의 Port A 에 붙는다. 쓰기와 되읽기가 한 포트를 나눠 쓰므로
    //   cal_addr 는 쓰기 주소와 되읽기 주소 중 하나를 고른다 (§3 의 assign cal_addr).
    output reg                             cal_we,
    output wire [8:0]                      cal_addr,
    output reg  [12:0]                     cal_din,
    input  wire [12:0]                     cal_data,
    ...
    // ★ 2026-09-16 추가 : Mode 0 확인(내부 루프백)용 히트 생성기 주기 [tdc_clk 클럭 수].
    //   0 이면 생성기가 멈춘다. 자세한 것은 tdc_test_top.v 의 DPS 생성기 주석.
    output reg  [15:0]                     ctrl_m0_period
);
    ...
    localparam [5:0] A_ID     = 6'h0,   // 0x00  R   매직값
                     ...
                     //   ->  히트율   ≈ f_RO / 63
                     //   ★ 2026-09-15 : 히트 발생기가 고정 ÷64(hit_random = ro_divider_cnt[5])에서
                     //     LFSR 난수 분주로 바뀌었다 (tdc_test_top.v 의 hd_lfsr/hd_cnt).
                     //     반주기마다 16~47 RO 주기를 뽑으므로 한 주기 평균이 63 RO 주기다.
                     //     [왜] 히트 간격이 클럭 주기의 정수배 근처에 머물면 히트 위상이 탭 전체에
                     //     고르게 퍼지지 않는다. 자세한 근거는 tdc_test_top.v 의 난수 분주 주석.
                     //   검증 : 2026-09-05 3단계(고정 ÷64 시절)에서 RO_CNT 168,194 -> 67.28 MHz
                     //          -> 히트율 1.051 MHz, 실측 200만발/1.9초 = 1.05 MHz.
                     //          난수 분주 뒤에는 평균값으로만 맞는다.
                     ...
                     // ★ 2026-09-16 : Mode 0 확인용 히트 생성기 주기 (맵 버전 8)
                     //   단위는 tdc_clk 클럭 수다. 50 -> 50 x 5000 ps = 250 ns 마다 히트 하나.
                     //   코어는 히트의 '하강 에지'에서 측정하므로 생성기는 사각파를 만들고
                     //   하강 에지 간격이 이 값이 된다 (tdc_fmcw_core_co.v:267).
                     A_M0PER  = 6'h12;  // 0x48  RW  [15:0]
    ...
    // ★ 2026-09-15 : 교정표는 0x2000 + 탭*4 (탭 0..511 -> 0x2000~0x27FC). 주소 [15:12] = 4'b0010.
    //   읽기 : RAM i번 칸의 13비트 값 (상위 19비트는 0)
    //   쓰기 : 하위 13비트만 쓴다. 시퀀서 BUSY 중에는 무시한다 — PS 는 되읽기로 확인할 것.
    //   히스토그램(비트 12)·캡처 버퍼(비트 15)와 겹치지 않는다.
    localparam integer CAL_SEL_BIT = 13;
    ...
    reg [15:0] m0_per_reg    = 16'd0;    // ★ 2026-09-16 : 0 = 생성기 정지
    reg        ctrl_upd_tgl;      // CTRL/PHASE 를 쓸 때마다 토글. CDC 용.
    reg        bvalid_r;
    reg  [8:0] cal_waddr = 9'd0;  // ★ 2026-09-15 : 교정표 쓰기 주소

    // ★ 2026-09-15 : 쓰기 수락 조건과 영역 판정을 고쳤다.
    //   (1) ~rd_busy — 교정표 RAM Port A 의 주소를 쓰기와 되읽기가 공유한다. 되읽기가 주소를
    //       걸어 둔 뒤 RAM 이 그 주소를 샘플링하기 전에 쓰기가 주소를 바꾸면 엉뚱한 칸이
    //       읽힌다. 읽기가 진행 중(hpend != 0)이면 AWREADY 를 늦춰 쓰기를 기다리게 한다.
    //       AXI 에서 READY 를 늦추는 것은 허용된다. 기존 레지스터 쓰기도 최대 2클럭 늦어질 뿐이다.
    //   (2) wr_reg — 종전에는 awaddr[7:2] 만 보고 썼다. 그래서 0x1008 에 써도 CTRL 이 바뀌었다
    //       (쓰는 코드가 없어 드러나지 않았을 뿐이다). 교정표 창을 넣으면 0x2008 에 쓰는 순간
    //       CTRL 이 바뀌므로, 레지스터 쓰기는 0x0000~0x0FFF 에서만 받는다.
    reg  [1:0] hpend = 2'b00;     // §3 의 읽기 진행 시프트 레지스터. 선언만 여기로 옮겼다.
    wire       rd_busy = |hpend;
    wire wr_go  = s_axi_awvalid & s_axi_wvalid & ~bvalid_r & ~rd_busy;
    wire wr_reg = (s_axi_awaddr[15:12] == 4'b0000);
    wire wr_cal = (s_axi_awaddr[15:12] == 4'b0010);
    ...
    always @(posedge s_axi_aclk) begin
        if (!s_axi_aresetn) begin
            ...
            m0_per_reg   <= 16'd0;        // ★ 2026-09-16
            ctrl_upd_tgl <= 1'b0;
            cal_we       <= 1'b0;         // ★ 2026-09-15
            cal_waddr    <= 9'd0;
            cal_din      <= 13'd0;
            bvalid_r     <= 1'b0;
        end else begin
            cal_we <= 1'b0;               // ★ 2026-09-15 : 교정표 쓰기는 1클럭 펄스
            if (wr_go) begin
                // ★ 2026-09-15 : 교정표 쓰기. st_d2[2] = 시퀀서 BUSY (§2 에서 2FF 동기화).
                //   측정 중에는 무시한다 — 타임스탬프 파이프라인이 매 클럭 같은 RAM 을 읽으므로
                //   쓰는 칸과 읽는 칸이 겹치면 그 히트의 교정값이 정해지지 않는다.
                //   무시해도 BRESP 는 OKAY 다. PS 는 되읽어서 확인한다.
                if (wr_cal && !st_d2[2]) begin
                    cal_we    <= 1'b1;
                    cal_waddr <= s_axi_awaddr[10:2];
                    cal_din   <= s_axi_wdata[12:0];
                end
                if (wr_reg && wr_word == A_CTRL) begin
                ...
                if (wr_reg && wr_word == A_PHASE) begin
                ...
                if (wr_reg && wr_word == A_TARGET) begin
                ...
                if (wr_reg && wr_word == A_M0PER) begin      // ★ 2026-09-16
                    if (s_axi_wstrb[0]) m0_per_reg[ 7:0] <= s_axi_wdata[ 7:0];
                    if (s_axi_wstrb[1]) m0_per_reg[15:8] <= s_axi_wdata[15:8];
                    ctrl_upd_tgl <= ~ctrl_upd_tgl;
                end
                if (wr_reg && wr_word == A_SETTLE) begin
    ...
    // ★ 2026-09-15 : hpend 선언은 §1 로 옮겼다 (쓰기 수락 조건 rd_busy 가 먼저 쓴다).
    reg       cap_rd = 1'b0;         // 이번 읽기가 캡처 버퍼인가
    wire      histo_busy = rd_busy;
    reg       cal_rd    = 1'b0;     // ★ 2026-09-15 : 이번 읽기가 교정표인가
    reg [8:0] cal_raddr = 9'd0;     // ★ 2026-09-15 : 교정표 되읽기 주소
    // ★ 2026-09-15 : 교정표 RAM Port A 주소. 쓰기 펄스(cal_we=1) 사이클엔 쓰기 주소, 아니면 되읽기 주소.
    //   둘이 같은 사이클에 RAM 에 닿지 않는 이유 : 되읽기는 rd_go(T)에 주소를 걸고 RAM 이 T+1 에
    //   샘플링한다. T 에는 아래 rd_go 의 ~wr_go 가, T+1 에는 §1 wr_go 의 ~rd_busy 가 쓰기를 막는다.
    assign cal_addr = cal_we ? cal_waddr : cal_raddr;
    ...
    wire rd_go   = s_axi_arvalid & ~rvalid_r & ~histo_busy & ~wr_go;  // ★ 2026-09-15 : 같은 사이클이면 쓰기 먼저
    wire is_cap  = s_axi_araddr[CAP_SEL_BIT];                        // 0x8000 이상
    wire is_hist = ~s_axi_araddr[CAP_SEL_BIT] & s_axi_araddr[HISTO_SEL_BIT];
    wire is_cal  = (s_axi_araddr[15:12] == 4'b0010);                 // ★ 2026-09-15 : 0x2000~0x2FFF
    ...
    always @(posedge s_axi_aclk) begin
        if (!s_axi_aresetn) begin
            ...
            if (rd_go && is_hist) begin
                ...
                cap_rd   <= 1'b1;
                cal_rd   <= 1'b0;         // ★ 2026-09-15
            end else if (rd_go && is_cal) begin
                // ★ 2026-09-15 : 0x2000 + 탭*4 -> 탭 번호는 주소 비트 [10:2].
                //   RAM Port A 출력이 레지스터드라 히스토그램과 같은 2클럭 경로(hpend)를 쓴다.
                cal_raddr <= s_axi_araddr[10:2];
                hpend     <= 2'b01;
                cap_rd    <= 1'b0;
                cal_rd    <= 1'b1;
                ...
                case (rd_word)
                    ...
                    A_M0PER:  rdata_r <= { 16'd0, m0_per_reg };          // ★ 2026-09-16
                    default:  rdata_r <= 32'hDEAD_0000;          // 없는 주소를 눈에 띄게
                endcase
                rvalid_r <= 1'b1;
            end else if (hpend[1]) begin
                rdata_r  <= cap_rd ? cap_data : cal_rd ? {19'd0, cal_data} : histo_data;  // ★ 2026-09-15 : 교정표 추가.  // BRAM 출력 유효 사이클
                rvalid_r <= 1'b1;
    ...
    always @(posedge tdc_clk or negedge tdc_rst_n) begin
        if (!tdc_rst_n) begin
            ...
            ctrl_m0_period   <= 16'd0;    // ★ 2026-09-16
        end else begin
            ...
            if (tgl_d2 ^ tgl_d3) begin
                ...
                ctrl_m0_period   <= m0_per_reg;   // ★ 2026-09-16
            end
        end
    end
